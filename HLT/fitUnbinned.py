...
def fitAndPlot(ws, obj, label, data=False):
	...
	ws.pdf('model').plotOn(frame,ROOT.RooFit.Components(ROOT.RooArgSet(ws.pdf("cb"))), ROOT.RooFit.Name("cb"),ROOT.RooFit.LineColor(ROOT.kRed),ROOT.RooFit.LineStyle(ROOT.kDashed))
	...
	if "2022" in label:	
		latex.DrawLatex(0.95, 0.96, "2022 (13.6 TeV)")
	else:	
		latex.DrawLatex(0.95, 0.96, "2023 (13.6 TeV)")
...
def runEff(ptBin, etaBin, era, data=True):
	...
	if data:
		if "2022" in era:
			tree.Add("AnalysedTree_data_2022_tau3mu_merged_2022v2.root/FinalTree")
		else:
			tree.Add("AnalysedTree_data_2023_tau3mu_merged_2023v2.root/FinalTree")
	else:	
...
#	histMu = ROOT.TH1F('muHist','muHist',40, 0.8,1.2)
	...
	datasetTrk = ROOT.RooDataSet('histTrkRoo','histTrkRoo',ROOT.RooArgSet(ws.var("mass")))
	datasetMu = ROOT.RooDataSet('histMuRoo','histMuRoo',ROOT.RooArgSet(ws.var("mass")))
	...
	ws.var("alphaL").setConstant(1)
	ws.var("nL").setConstant(1)
	...
	print ("Total Yields: %d, %d"%(numTk, numMu))
	print ("Signal Fractions: %.3f %.3f"%(fSigTk, fSigMu))
	print ("Eff (weighted and raw): %.3f %.3f"%(eff, eff2))
